        ...
        actions: {
          edit: {
            ...
            handler: async (request: any, response: any, context: any) => {
              ...
              if (record.isValid()) {
                //@ts-ignore
                UserStats.updateStatsAfterUpdateRecipeStatus(record.params._authorId, status, previousStatus);
                return {
                  redirectUrl: h.resourceUrl({ resourceId: resource._decorated?.id() || resource.id() }),
                  notice: {
                    message: translateMessage("successfullyUpdated", resource.id()),
          ...
          AcceptProduct: {
            ...
            handler: async (request: any, response: any, context: any) => {
              ...
              //@ts-ignore
              UserStats.updateStatsAfterUpdateRecipeStatus(record.params._authorId, "public", "in_progress");
              return {
                redirectUrl: h.resourceUrl({ resourceId: resource._decorated?.id() || resource.id() }),
                notice: {
                  message: translateMessage("successfullyUpdated", resource.id()),
